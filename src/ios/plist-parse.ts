...
function toString(plistBytes: Uint8Array, objInfo: number, isUTF16: boolean = false): string {
    let length = objInfo;
    let stringOffset = 0;
    if (objInfo === 0xf) {
        const intType = (plistBytes[0] & 0xf0) >> 4;
...
function toArray(plistBytes: Uint8Array, objInfo: number, objectRefSize: number, fullBytes: Uint8Array) {
    let length = objInfo;
    let arrayOffset = 0;
    if (objInfo === 0xf) {
        const intType = (plistBytes[0] & 0xf0) >> 4;
    ...
    for (var i = 0; i < length; i++) {
        var objRef = parseInt(Tools.toHex(plistBytes.slice(arrayOffset + i * objectRefSize, arrayOffset + (i + 1) * objectRefSize)), 16);
        array.push(parseObject(fullBytes, offsetTable[objRef]));
      }
      return array;
}

function toDictionary(plistBytes: Uint8Array, objInfo: number, objectRefSize: number, fullBytes: Uint8Array) {  
    let length = objInfo;
    let dicOffset = 0;
    if (objInfo == 0xf) {
        const intType = (plistBytes[0] & 0xf0) >> 4;
    ...
    for (let i = 0; i < length; i ++) {
        const keyRef = parseInt(Tools.toHex(plistBytes.slice(dicOffset + i * objectRefSize, dicOffset + (i + 1) * objectRefSize)), 16);
        const valRef = parseInt(Tools.toHex(plistBytes.slice(dicOffset + (length * objectRefSize) + i * objectRefSize, dicOffset + (length * objectRefSize) + (i + 1) * objectRefSize)), 16);
...
export class PList {
    ...
    constructor(public info: {[key in string]: string | any}) { }
}
